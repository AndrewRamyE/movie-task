...
<template>
  ...
  <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
    <div>
      <nav v-for="link in links" class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
        <Link :href="link.url" v-html="link.label" v-if="link.url" :class="`relative inline-flex items-center rounded-l-md border border-gray-300 ${link.label == currentPage ? 'bg-indigo-50' : 'bg-white'} px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20`" />
     ...
     <!-- <link v-for="link in links" :href="link.url" v-html="link.label" /> -->
</template>
